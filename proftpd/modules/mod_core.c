...
MODRET set_cephfsdir(cmd_rec *cmd) {
  if (cmd->argc-1 != 1)
    CONF_ERROR(cmd, "wrong number of parameters");
  CHECK_CONF(cmd, CONF_ROOT);
  char *cephfs_dir = NULL;
  if (pr_fs_valid_path(cmd->argv[1]) < 0)
    CONF_ERROR(cmd, "must be an absolute path");

  cephfs_dir = pstrdup(cmd->server->pool, cmd->argv[1]);
  init_cephfs(cephfs_dir);

  if (register_cephfs("/") < 0) {
      CONF_ERROR(cmd, "set_cephfsdir faided\n");
  }
  //add Cephfs {cephfs_dir} to global config parm
  add_config_param_str(cmd->argv[0], 1, cmd->argv[1]);
  return PR_HANDLED(cmd);
}
