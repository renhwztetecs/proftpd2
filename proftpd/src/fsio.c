...
extern struct ceph_mount_info *cephfs_mount;

int init_cephfs(char *path) {
  ...
  if (ceph_create(&cephfs_mount, NULL)<0) {
       pr_trace_msg(trace_channel, 12, "error: unable to create mount point");
       return -1;
  }

  if (ceph_conf_read_file(cephfs_mount, "/etc/ceph/ceph.conf") < 0) {
       pr_trace_msg(trace_channel, 12, "error: unable to read /etc/ceph/ceph.conf");
       ceph_release(cephfs_mount);
       cephfs_mount = NULL;
       return -1;
  }

  if (ceph_mount(cephfs_mount, path) < 0) {
       pr_trace_msg(trace_channel, 12, "error: unable to mount cephfs path:%s retry again", path);
       if (ceph_mount(cephfs_mount, "/") < 0)
       {
           pr_trace_msg(trace_channel, 12, "error: unable to mount cephfs");
           ceph_release(cephfs_mount);
           cephfs_mount = NULL;
           return -1;
       }
  ...
  pr_trace_msg(trace_channel, 12, "cephfs init success:%p mountpoint:%s", cephfs_mount ,path);
  return 0;
}
...
/*cephfs-statx to stat struct convertion*/
static int to_stat(struct ceph_statx *sx, struct stat *st) {
  if (sx == NULL || st == NULL ) {
     return -1;
  }

  st->st_dev     = sx->stx_dev;
  st->st_ino     = sx->stx_ino;
  st->st_mode    = sx->stx_mode;
  st->st_nlink   = sx->stx_nlink;
  st->st_uid     = sx->stx_uid;
  st->st_gid     = sx->stx_gid;
  st->st_rdev    = sx->stx_rdev;
  st->st_size    = sx->stx_size;
  st->st_blksize = sx->stx_blksize;
  st->st_blocks  = sx->stx_blocks;
  st->st_atim    = sx->stx_atime;
  st->st_mtim    = sx->stx_mtime;
  st->st_ctim    = sx->stx_ctime;
...
static int ceph_sys_stat(pr_fs_t *fs, const char *path, struct stat *sbuf) {
  int res;
  pr_trace_msg(trace_channel, 8, "stat(%p, %s)", cephfs_mount, path);
  struct ceph_statx stx;
  res = ceph_statx(cephfs_mount, path, &stx, CEPH_STATX_ALL_STATS, 0);
  to_stat(&stx, sbuf);
  pr_trace_msg(trace_channel, 8, "stat(...) = %d", res);
  return res;
}
...
static int ceph_sys_fstat(pr_fh_t *fh, int fd, struct stat *sbuf) {
  int res;
  pr_trace_msg(trace_channel, 8, "fstat(%p, %d)", cephfs_mount, fd);
  struct ceph_statx stx;
  res = ceph_fstatx(cephfs_mount, fd, &stx, CEPH_STATX_ALL_STATS, 0);
  to_stat(&stx, sbuf);
  pr_trace_msg(trace_channel, 8, "fstat(...) = %d", res);
  return res;
}
...
static int ceph_sys_lstat(pr_fs_t *fs, const char *path, struct stat *sbuf) {
  int res;
  pr_trace_msg(trace_channel, 8, "lstat(%p, %s)", cephfs_mount, path);
  struct ceph_statx stx;
  res = ceph_statx(cephfs_mount, path, &stx, CEPH_STATX_ALL_STATS, 0);
  to_stat(&stx, sbuf);
  pr_trace_msg(trace_channel, 8, "lstat(...) = %d", res);
  return res;
}
...
static int ceph_sys_rename(pr_fs_t *fs, const char *rnfm, const char *rnto) {
  int res;
  ...
  pr_trace_msg(trace_channel, 8, "rename(%p, %s, %s)", cephfs_mount, rnfm, rnto);
  res = ceph_rename(cephfs_mount, rnfm, rnto);
  pr_trace_msg(trace_channel, 8, "rename(...) = %d", res);
  return res;
}
...
static int ceph_sys_unlink(pr_fs_t *fs, const char *path) {
  int res;
  ...
  pr_trace_msg(trace_channel, 8, "unlink(%p, %s)", cephfs_mount, path);
  res = ceph_unlink(cephfs_mount, path);
  pr_trace_msg(trace_channel, 8, "unlink(...) = %d", res);
  return res;
}
...
static int ceph_sys_open(pr_fh_t *fh, const char *path, int flags) {
  int res;
  ...
  pr_trace_msg(trace_channel, 8, "open(%p, %s, %d)", cephfs_mount, path, flags);
  res = ceph_open(cephfs_mount, path, flags, PR_OPEN_MODE);
  pr_trace_msg(trace_channel, 8, "open(...) = %d", res);
  return res;
}
...
static int ceph_sys_close(pr_fh_t *fh, int fd) {
  int res;
  pr_trace_msg(trace_channel, 8, "close(%p, %d)", cephfs_mount, fd);
  res = ceph_close(cephfs_mount, fd);
  pr_trace_msg(trace_channel, 8, "open(...) = %d", res);
  return res;
}
...
static int ceph_sys_read(pr_fh_t *fh, int fd, char *buf, size_t size) {
  int res;
  pr_trace_msg(trace_channel, 8, "read(%p, %d, %lu)", cephfs_mount, fd, size);
  res = ceph_read(cephfs_mount, fd, buf, size, -1);
  pr_trace_msg(trace_channel, 8, "read(...) = %d", res);
  return res;
}
...
static int ceph_sys_write(pr_fh_t *fh, int fd, const char *buf, size_t size) {
  int res;
  pr_trace_msg(trace_channel, 8, "write(%p, %d, %lu)", cephfs_mount, fd, size);
  res = ceph_write(cephfs_mount, fd, buf, size, -1);
  pr_trace_msg(trace_channel, 8, "write(...) = %d", res);
  return res;
}
...
static off_t ceph_sys_lseek(pr_fh_t *fh, int fd, off_t offset, int whence) {
  int res;
  pr_trace_msg(trace_channel, 8, "lseek(%p, %d, %lu, %d)", cephfs_mount, fd, offset, whence);
  res = ceph_lseek(cephfs_mount, fd, offset, whence);
  pr_trace_msg(trace_channel, 8, "lseek(...) = %d", res);
  return res;
}
...
static int ceph_sys_link(pr_fs_t *fs, const char *target_path,
    const char *link_path) {
  int res;
  ...
  pr_trace_msg(trace_channel, 8, "link(%p, %s, %s)", cephfs_mount, target_path, link_path);
  res = ceph_link(cephfs_mount, target_path, link_path);
  pr_trace_msg(trace_channel, 8, "link(...) = %d", res);
  return res;
}
...
static int ceph_sys_symlink(pr_fs_t *fs, const char *target_path,
    const char *link_path) {
  int res;
  ...
  pr_trace_msg(trace_channel, 8, "symlink(%p, %s, %s)", cephfs_mount, target_path, link_path);
  res = ceph_symlink(cephfs_mount, target_path, link_path);
  pr_trace_msg(trace_channel, 8, "symlink(...) = %d", res);
  return res;
}
...
static int ceph_sys_readlink(pr_fs_t *fs, const char *path, char *buf,
    size_t buflen) {
  int res;
  pr_trace_msg(trace_channel, 8, "readlink(%p, %s, %lu)", cephfs_mount, path, buflen);
  res = ceph_readlink(cephfs_mount, path, buf, buflen);
  pr_trace_msg(trace_channel, 8, "readlink(...) = %d", res);
  return res;
}
...
static int ceph_sys_ftruncate(pr_fh_t *fh, int fd, off_t len) {
  int res;
  pr_trace_msg(trace_channel, 8, "ftruncate(%p, %d, %lu)", cephfs_mount, fd, len);
  res = ceph_ftruncate(cephfs_mount, fd, len);
  pr_trace_msg(trace_channel, 8, "ftruncate(...) = %d", res);
  return res;
}
...
static int ceph_sys_truncate(pr_fs_t *fs, const char *path, off_t len) {
  int res;
  ...
  pr_trace_msg(trace_channel, 8, "truncate(%p, %s, %lu)", cephfs_mount, path, len);
  res = ceph_truncate(cephfs_mount, path, len);
  pr_trace_msg(trace_channel, 8, "truncate(...) = %d", res);
  return res;
}
...
static int ceph_sys_chmod(pr_fs_t *fs, const char *path, mode_t mode) {
  int res;
  ...
  pr_trace_msg(trace_channel, 8, "chmod(%p, %s, %d)", cephfs_mount, path, mode);
  res = ceph_chmod(cephfs_mount, path, mode);
  pr_trace_msg(trace_channel, 8, "chmod(...) = %d", res);
  return res;
}
...
static int ceph_sys_fchmod(pr_fh_t *fh, int fd, mode_t mode) {
  int res;
  pr_trace_msg(trace_channel, 8, "fchmod(%p, %d, %d)", cephfs_mount, fd, mode);
  res = ceph_fchmod(cephfs_mount, fd, mode);
  pr_trace_msg(trace_channel, 8, "fchmod(...) = %d", res);
  return res;
}
...
static int ceph_sys_chown(pr_fs_t *fs, const char *path, uid_t uid, gid_t gid) {
  int res;
  ...
  pr_trace_msg(trace_channel, 8, "chown(%p, %s, %d, %d)", cephfs_mount, path, uid, gid);
  res = ceph_chown(cephfs_mount, path, uid, gid);
  pr_trace_msg(trace_channel, 8, "chown(...) = %d", res);
  return res;
}
...
static int ceph_sys_fchown(pr_fh_t *fh, int fd, uid_t uid, gid_t gid) {
  int res;
  pr_trace_msg(trace_channel, 8, "fchown(%p, %d, %d, %d)", cephfs_mount, fd, uid, gid);
  res = ceph_fchown(cephfs_mount, fd, uid, gid);
  pr_trace_msg(trace_channel, 8, "fchown(...) = %d", res);
  return res;
}
...
static int ceph_sys_lchown(pr_fs_t *fs, const char *path, uid_t uid, gid_t gid) {
  int res;
  ...
  pr_trace_msg(trace_channel, 8, "lchown(%p, %s, %d, %d)", cephfs_mount, path, uid, gid);
  res = ceph_lchown(cephfs_mount, path, uid, gid);
  pr_trace_msg(trace_channel, 8, "lchown(...) = %d", res);
  return res;
}
...
static int ceph_sys_fsync(pr_fh_t *fh, int fd) {
  int res;

#ifdef HAVE_FSYNC
  pr_trace_msg(trace_channel, 8, "fsync(%p, %d)", cephfs_mount, fd);
  res = ceph_fsync(cephfs_mount, fd, 0);
  pr_trace_msg(trace_channel, 8, "fsync(...) = %d", res);
#else
  errno = ENOSYS;
  res = -1;
#endif /* HAVE_FSYNC */
...
static int ceph_sys_chdir(pr_fs_t *fs, const char *path) {
  int res;
  pr_trace_msg(trace_channel, 8, "chdir(%p, %s)", cephfs_mount, path);
  res = ceph_chdir(cephfs_mount, path);
  pr_trace_msg(trace_channel, 8, "chdir(...) = %d", res);
  if (res < 0) {
    return -1;
  }
...
static int ceph_sys_chroot(pr_fs_t *fs, const char *path) {

  session.chroot_path = (char *) path;
    pr_trace_msg(trace_channel, 10, "session.chroot_path %s", session.chroot_path);
  return 0;
}
...
static void *ceph_sys_opendir(pr_fs_t *fs, const char *path) {
  pr_trace_msg(trace_channel, 8, "opendir(%p, %s)", cephfs_mount, path);
  struct ceph_dir_result *res = NULL;
  ceph_opendir(cephfs_mount, path, &res);
  pr_trace_msg(trace_channel, 8, "opendir(...) = %p", res);
  return (DIR *)res;
}
...
static int ceph_sys_closedir(pr_fs_t *fs, void *dir) {
  int res;
  pr_trace_msg(trace_channel, 8, "closedir(%p, %p)", cephfs_mount, dir);
  res = ceph_closedir(cephfs_mount, (struct ceph_dir_result *)dir);
  pr_trace_msg(trace_channel, 8, "closedir(...) = %d", res);
  return res;
}
...
static struct dirent *ceph_sys_readdir(pr_fs_t *fs, void *dir) {
  pr_trace_msg(trace_channel, 8, "readdir(%p, %p)\n", cephfs_mount, dir);
  struct dirent *res = NULL;
  res = ceph_readdir(cephfs_mount, (struct ceph_dir_result *)dir);
  pr_trace_msg(trace_channel, 8, "readdir(...) = %p\n", res);
  return res;
}
...
static int ceph_sys_mkdir(pr_fs_t *fs, const char *path, mode_t mode) {
  int res;
  ...
  dir_umask = get_param_ptr(TOPLEVEL_CONF, "Umask", FALSE);
  if (dir_umask != NULL)
  {
    mask = *dir_umask;
  }
  ...
  mode = (mode & ~mask);
  pr_trace_msg(trace_channel, 8, "mkdir(%p, %s, mode %04o umask(%04o))\n", cephfs_mount, path, mode, mask);
  res = ceph_mkdir(cephfs_mount, path, mode);
  pr_trace_msg(trace_channel, 8, "mkdir(...) = %d\n", res);
  return res;
}
...
static int ceph_sys_rmdir(pr_fs_t *fs, const char *path) {
  int res;
  ...
  pr_trace_msg(trace_channel, 8, "rmdir(%p, %s)\n", cephfs_mount, path);
  res = ceph_rmdir(cephfs_mount, path);
  pr_trace_msg(trace_channel, 8, "rmdir(...) = %d\n", res);
  return res;
}
...
pr_fs_t *pr_get_fs(const char *path, int *exact) {
  ...
  if (path == NULL) {
    ...
    pr_trace_msg(trace_channel, 8, "pr_get_fs: path(%s)", path);
  ...
  for (i = 0; i < fs_map->nelts; i++) {
    ...
    pr_trace_msg(trace_channel, 8, "pr_get_fs:strcmp(fs->fs_path(%s), path(%s))", fs->fs_path,path);
...
int pr_fs_use_encoding(int bool_flag) {
...
int register_cephfs(char *path) {

  if (path == NULL) {
      errno = EINVAL;
      return -1;
  }
  pr_trace_msg(trace_channel, 12, "register_cephfs path:%s", path);
  pr_fs_t *ceph_fs;
  int xerrno = 0;
  ceph_fs = pr_create_fs(permanent_pool, "cephfs");
  xerrno = errno;

  if (ceph_fs != NULL) {
    if (pr_insert_fs(ceph_fs, path) == FALSE) {
      xerrno = errno;

      pr_trace_msg(trace_channel, 5, "error inserting FS CEPH at path /cephftp");
      destroy_pool(ceph_fs->fs_pool);
      ceph_fs->fs_pool = NULL;

      errno = xerrno;
      return -1;
    }
  } else {
    pr_trace_msg(trace_channel, 12, " creating FS CEPH success");
  }

  ceph_fs->stat = ceph_sys_stat;
  ceph_fs->fstat = ceph_sys_fstat;
  ceph_fs->lstat = ceph_sys_lstat;
  ceph_fs->rename = ceph_sys_rename;
  ceph_fs->unlink = ceph_sys_unlink;
  ceph_fs->open = ceph_sys_open;
  ceph_fs->close = ceph_sys_close;
  ceph_fs->read = ceph_sys_read;
  ceph_fs->write = ceph_sys_write;
  ceph_fs->lseek = ceph_sys_lseek;
  ceph_fs->link = ceph_sys_link;
  ceph_fs->readlink = ceph_sys_readlink;
  ceph_fs->symlink = ceph_sys_symlink;
  ceph_fs->ftruncate = ceph_sys_ftruncate;
  ceph_fs->truncate = ceph_sys_truncate;
  ceph_fs->chmod = ceph_sys_chmod;
  ceph_fs->fchmod = ceph_sys_fchmod;
  ceph_fs->chown = ceph_sys_chown;
  ceph_fs->fchown = ceph_sys_fchown;
  ceph_fs->lchown = ceph_sys_lchown;
  ceph_fs->fsync = ceph_sys_fsync;
  ceph_fs->chdir = ceph_sys_chdir;
  ceph_fs->opendir = ceph_sys_opendir;
  ceph_fs->closedir = ceph_sys_closedir;
  ceph_fs->readdir = ceph_sys_readdir;
  ceph_fs->mkdir = ceph_sys_mkdir;
  ceph_fs->rmdir = ceph_sys_rmdir;
  ceph_fs->chroot = ceph_sys_chroot;
  return 0;
