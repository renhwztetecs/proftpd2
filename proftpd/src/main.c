...
struct ceph_mount_info *cephfs_mount = NULL;
...
static void fork_server(int fd, conn_t *l, unsigned char no_fork) {
  ...
  if (no_fork == FALSE) {
    ...
    case 0: /* child */
      ...
      if (cephfs_mount != NULL) {
        free(cephfs_mount);
        cephfs_mount=NULL;
      }
      char * cephfs_dir;
      cephfs_dir = get_param_ptr((main_server ? main_server->conf : NULL), "Cephfs", FALSE);
      if (cephfs_dir == NULL)
      {
          pr_log_pri(PR_LOG_NOTICE, "CEPH:get cephfs_dir failed");
          break;
      }
      else
      {
          init_cephfs(cephfs_dir);
      }
      pr_log_pri(PR_LOG_NOTICE, "reinitfs end mount:%p", cephfs_mount);
